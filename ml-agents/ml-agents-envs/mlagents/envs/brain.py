...
class BrainInfo:
    def __init__(self, visual_observation, vector_observation, text_observations, memory=None,
                 ...
                 custom_observations=None, api_connection=False):
        """
    ...
    def merge(self, other):
        ...
        self.max_reached = safe_concat_lists(
            self.max_reached, other.max_reached)
    ...
    def from_agent_proto(agent_info_list, brain_params):
        ...
        if len(agent_info_list) == 0:
            vector_obs = np.zeros(
                (0, brain_params.vector_observation_space_size *
        ...
        brain_info = BrainInfo(
            ...
            memory=memory,
            reward=[x.reward if not np.isnan(
                x.reward) else 0 for x in agent_info_list],
            agents=[x.id for x in agent_info_list],
            local_done=[x.done for x in agent_info_list],
            vector_action=np.array(
                [x.stored_vector_actions for x in agent_info_list]),
            text_action=[list(x.stored_text_actions) for x in agent_info_list],
            max_reached=[x.max_step_reached for x in agent_info_list],
            custom_observations=[
                x.custom_observation for x in agent_info_list],
